...
public:
    ...
    void maximumPlanarSubset(){
        ...
        // M[2*n-1][2*n-1]={}; // This will initialize all elements to zero（把左下三角形也變成0不然會重複算到)同一條chord //創建一個DP用表格 共2n-1個數
        //cout<<"ok8"<< endl;
        // S[2*n-1][2*n-1];//用來紀錄各個M[i,j]是來自case幾，去看他們是從誰來的
        //cout<<"ok9"<< endl;
        //<Debug2-1> 要用assign!
        M.assign(2 * n, std::vector<int>(2 * n, 0)); //<Debug2-2> 0~2n-1共有2n個數！!
...
private:
    ...
    std::vector<std::vector<int>> maximumPlanarSubset_chord;
    ...
    void trace_maximumPlanarSubset_chord(int i,int j){
        ...
        }else if(S[i][j]==3){//case3 //ij上面沒線, 來自M[i][j]=M[i][j-1]; //其實就是case1
            trace_maximumPlanarSubset_chord(i,j-1);
